...
const Dashboard = () => {
  ...
  const reloadStats = async () => {
    ...
    try {
      ...
      // const h = await api.get("/api/history?limit=10"); setRecent(h.data?.data || []);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="dashboard-page">
      ...
      <section className="dash-hero">
        <div className="hero-inner">
          ...
          <div className="actions">
            <button className="btn btn-primary" onClick={goPrograms}>
              Commencer une séance
            </button>
            {coachMode && (
              <button className="btn btn-outline" onClick={goCreate}>
                Créer un programme
              </button>
            )}
          </div>
          ...
          <>
            ...
            <div className="dash-card kpi">
              ...
              <div className="kpi-value">
                {Math.floor(durationTotalMin / 60)}h {durationTotalMin % 60}m
              </div>
            ...
            <div className="dash-card progress">
              ...
              <div className="progress-info">
                ...
                <div className="desc">
                  {weekMinutes} / {weekTarget} min cette semaine
                </div>
              </div>
            </div>
          </>
      ...
      <section className="my-programs">
        ...
        {loading ? (
          ...
          <div className="empty">
            Tu n’es inscrit à aucun programme. Parcours la bibliothèque pour commencer.
            <div>
              <button className="btn btn-primary" onClick={goPrograms}>
          ...
          <div className="grid">
            {featured.map((p) => (
              <article className="dash-card program-card" key={p.id}>
                <div className="top">
                  <h4 className="title">{p.title}</h4>
                  {p.level && <span className={`badge ${p.level}`}>{p.level}</span>}
                </div>
                {p.description && <p className="desc">{p.description}</p>}
                <div className="bottom">
                  <div className="coach">
                    <div className="av">{initials(p.coach?.name || "C")}</div>
                    <span>{p.coach?.name || "Coach"}</span>
                  </div>
                  <button
                    className="btn btn-outline"
                    onClick={() => setActiveProgramme(p)}   // 👈 Ouvrir le panneau de séance
                  >
                    Continuer
                  </button>
                </div>
      ...
      {/* DERNIÈRES ACTIVITÉS (si tu branches /api/history) */}
      <section className="recent">
        ...
        {loading ? (
          ...
          <ul className="history">
            {recent.map((r, i) => (
              <li key={i}>
                <span className="when">{new Date(r.createdAt).toLocaleString()}</span>
                <span className="txt">
                  {r.title || r.exercice?.name || "Séance"} — {r.reps || r.durationMin || ""}
                </span>
              </li>
            ))}
          </ul>
        ) : (
          <div className="empty small">Aucune activité récente à afficher.</div>
        )}
      </section>
      ...
      {activeProgramme && (
        <WorkoutSession
          ...
          onClose={async (refresh = false) => {
            ...
            if (refresh) {
              setLoading(true);
              await reloadStats(); // rafraîchir les stats après enregistrement
            }
          }}
        />
      )}
